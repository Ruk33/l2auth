                       ...
                       size_t n)
{
    ...
    case PLATFORM_SOCKET_NEW_CONNECTION:
        client_socket = platform_socket_new();
        ...
        if (!platform_socket_accept(client_socket, socket)) {
            // util_recycle_id(g_socket_instances, free_socket);
...
static int init_timer(void)
{
    ...
    if (!platform_timer_init(timer, 33, 1)) {
        printf("game server unable to initialize timer.\n");
        return 0;
    }
...
int main(/* int argc, char **argv */)
{
    ...
    // util_recycle_id_get(&free_socket, g_socket_instances);
    // g_sockets[free_socket] = platform_socket_new();
    server_socket = platform_socket_new();
    ...
    if (!platform_socket_init(server_socket, 7777, MAX_CLIENTS)) {
        printf("game server socket unable to initialize.\n");
        return 1;
    }

    if (!init_timer()) {
        printf("unable to initalize timer.\n");
        return 1;
    }
    ...
    if (!platform_socket_listen(0, 0, on_request)) {
