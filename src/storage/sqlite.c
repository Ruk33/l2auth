...
#define sqlite_bind_int(stmt, key, value)                                      \
    sqlite_ok_or_log(sqlite3_bind_int(stmt,                                    \
                                      sqlite3_bind_parameter_index(stmt, key), \
                                      value))
...
#define sqlite_cpy_text(dest, stmt, col)                  \
    util_cpy_str((char *) dest,                           \
                 (char *) sqlite3_column_text(stmt, col), \
                 sizeof(dest),                            \
                 sizeof(dest))

#define ACCOUNT_FROM_USERNAME_QUERY                                       \
    "select username, encrypted_password from accounts where username = " \
    ":username limit 1;"

#define ACCOUNT_CREATE_QUERY                                                 \
    "insert into accounts(username, encrypted_password) values (:username, " \
    ":encrypted_password);"

#define SERVERS_QUERY                                                     \
    "select id, ip, port, age_limit, pvp, players, max_players, status, " \
    "extra, brackets from servers limit :limit;"
...
#define CHARACTERS_COLUMNS                                                 \
    ...
    "p_attack_speed, m_attack_speed, movement_speed_multiplier, "          \
...
#define CHARACTERS_CREATE_QUERY                                        \
    ...
    ":p_attack, :m_attack, :p_def, :m_def, :evasion_rate, :accuracy, " \
    ":critical_hit, "                                                  \
    ":run_speed, :walk_speed, :p_attack_speed, :m_attack_speed, "      \
...
int storage_get_account(struct ls_account *dest, char *username)
{
    ...
    if (result) {
        ...
        util_cpy_bytes(dest->encrypted_password,
