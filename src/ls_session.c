...
static void ls_session_blowfish_encrypt(struct ls_blowfish *bf,
                                        byte_t *dest,
                                        byte_t *src,
                                        size_t n)
{
...
static void ls_session_blowfish_decrypt(struct ls_blowfish *bf,
                                        byte_t *dest,
                                        byte_t *src,
                                        size_t n)
{
...
struct ls_session *ls_session_new(struct ls_state *ls,
...
struct ls_session *ls_session_find(struct ls_state *ls,
                                   struct platform_socket *socket)
{
    assert(ls);
    assert(socket);
...
void ls_session_send_packet(struct ls_state *ls,
...
void ls_session_encrypt_packet(struct ls_session *session,
                               byte_t *dest,
                               packet_t *src)
{
    u16_t body_size      = 0;
    ...
    *((u16_t *) dest) = encrypted_size;
    ls_session_blowfish_encrypt(session->blowfish, dest + 2, src + 2, body_size);
}

void ls_session_decrypt_packet(struct ls_session *session,
                               packet_t *dest,
                               byte_t *src)
{
    u16_t src_size      = 0;
    ...
    *((u16_t *) dest) = *((u16_t *) src);
    ...
    ls_session_blowfish_decrypt(session->blowfish,
                                dest_body,
                                src_body,
                                src_size);
