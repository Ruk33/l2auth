...
/**
 * TODO: This is not correct, the client
 * may send a request bigger than the max size
 * of a packet (meaning, multiple packets
 * at the same time)
 */
...
static void init_client(struct Client *client, int fd)
{
        ...
        client->fd            = fd;
        client->active        = 1;
        client->read_buffer   = calloc(1, MAX_REQUEST_SIZE);
        client->write_buffer  = calloc(1, MAX_REQUEST_SIZE);
...
ssize_t socket_send(int fd, unsigned char *response, size_t response_size)
{
        ...
        memcpy(client->write_buffer + client->to_be_written,
               response,
               response_size);
        ...
        sent_bytes =
                send(fd,
                     ...
                     client->to_be_written - client->written_bytes,
                     0);
...
static int socket_handle_events(socket_conn_t *conn, int epoll_fd)
{
        ...
        for (int i = 0; i < events_count; i++) {
                ...
                if (events[i].events & EPOLLIN) {
                        ...
                        tmp = recv(
                                fd, client->read_buffer, MAX_REQUEST_SIZE, 0);
...
int socket_handle_requests(socket_conn_t *conn)
{
        ...
        events  = calloc(1, sizeof(*events) * MAX_REQUESTS);
        clients = calloc(1, sizeof(*clients) * MAX_REQUESTS);
