...
typedef void *(*malloc_cb)(size_t how_much);

typedef void (*free_cb)(void *mem);
...
struct GameServerLib
{
        ...
        void (*disconnect)(void *server_data, int client_id);
};

static struct GameServerLib *game_server_lib = NULL;

static time_t lib_last_modified(void)
{
...
static void game_server_lib_load_if_required(void)
{
        ...
        if (game_server_lib->library)
                dlclose(game_server_lib->library);
        ...
        DLEXPR_OR_ERR(
            game_server_lib->request = dlsym(
                game_server_lib->library,
                "world_client_request"));

        DLEXPR_OR_ERR(
            game_server_lib->disconnect = dlsym(
                game_server_lib->library,
                "world_client_disconnect"));
}
