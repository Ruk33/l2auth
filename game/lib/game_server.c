...
void game_server_new_request(int client, byte_t *request, ssize_t request_size, void *data, host_alloc alloc_memory, host_dealloc dealloc_memory, host_send_response_cb send_response, host_disconnect_cb disconnect_connection)
{
        assert(request);
        assert(data);
        assert(alloc_memory);
