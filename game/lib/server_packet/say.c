...
void server_packet_say(packet *dest, character_t *character, l2_string_type_t msg_type, char *msg, size_t msg_size)
{
        ...
        l2_string_from_char(say_packet.sender_name, character->name, strlen(character->name) + 1);
        l2_string_from_char(say_packet.msg, msg, msg_size < max_msg_size ? msg_size : max_msg_size);
