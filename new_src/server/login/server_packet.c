...
void packet_init_to(struct packet *dest, struct packet_init *src)
{
    ...
    packet_body_append(dest, &BFA(src->session_id.buf));
    packet_body_append(dest, &BFA(src->protocol.buf));
    packet_body_append(dest, &BFA(src->modulus.buf));
}
...
void packet_ok_to(struct packet *dest, struct packet_ok *src)
{
    ...
    packet_body_append(dest, &BFA(after_key));
}
